...
```genefinder.finder.filter(taglist,lengthlimit,threshold,rid = ['ttn-1','cla-1'])```
...
```genefinder.finder.plot(result,size,c='Reds',sub_c='RdPu',intersection_num = 1)```
#### c = color map used in the major graph
#### sub_c = color map used in other graphs
#### intersection_num = 1
###### For example: If you think gene A is a suppressor, it means that in samples with other mutations,there is less oppotunity for them to carry a gene A mutation. intersection_num means the least number of the sample number you allow to carry more than one suppressor.
###### You can now find the intersection by click on the bubble of the gene on the graph

### Other changes
```genefinder.finder.intersection(result,samplelist)```
###### You can use this function to find genes not in these samples. Return a Dataframe
```genefinder.finder.gsamplename(result,gene)```
###### You can get a list of your sample carry this gene.
```genefinder.reader.get_impact(taglist)```
...
```genefinder.getfile(filepath,filename)```
